...
DEFAULT_ROOT = path.abspath("../examples/example-files/golang/")
...
def initialize_lsp(json_rpc: pylspclient.JsonRpcEndpoint) -> pylspclient.LspClient:
    ...
    process_id = None
    root_path = None
    root_uri = to_uri(DEFAULT_ROOT, uri_scheme="file")
...
def get_document_symbols(file_path: str, lsp_client: pylspclient.LspClient):
    ...
    languageId = LanguageIdentifier.GO
    version = 1
...
file_path = "test.go"
symbols = get_document_symbols("test.go", client)
