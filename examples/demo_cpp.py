...
DEFAULT_ROOT = path.abspath("../examples/example-files/cpp/")
...
def get_document_symbols(file_path: str, lsp_client: pylspclient.LspClient):
    ...
    uri = to_uri(relative_file_path, uri_scheme="file")
...
def get_function_definition(lsp_client: pylspclient.LspClient, file_path: str, function_name: str):
    ...
    # Ensure the document is opened in the LSP session
    languageId = LanguageIdentifier.CPP  
    version = 1
    lsp_client.didOpen(TextDocumentItem(uri=uri, languageId=languageId, version=version, text=file_content))
...
file_path = "test.cpp"
symbols = get_document_symbols(file_path, client)
